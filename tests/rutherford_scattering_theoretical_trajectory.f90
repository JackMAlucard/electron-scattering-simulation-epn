!===============================================================================
! Program    : rutherford_scattering_theoretical_trajectory
! Purpose    : This program is intended as a proof of concept to become familiar
!              with Rutherford Scattering, and how the different geometric and
!              physical parameters used impact the resulting trajectories.
! Description:
...
! Output     :
!   For each simulation,
!     - A '.dat' file with the theoretical trajectory:
...
program rutherford_scattering_theoretical_trajectory
	...
	real(dp) :: scat_angle_theoretical, scat_angle_plot
	...
	do k = 1, num_simulations
		...
		output_file = 'K0_'//K0_char_trim//'keV_x0_'//x0_char_trim// &
			'Å_b_'//b_char_trim//'Å_dt_'//dt_char_trim//'aut_theoretical_trajectory'
	...
	subroutine compute_theoretical_trajectory &
		...
		do i = 0, num_plot_ploints
			...
			if (is_ri_inf .or. abs(ri) > ri_max) then
				print*, 'TRIMMING! ri initial:', ri
				ri = ri_max
				print*, 'TRIMMING! ri final:', ri
			end if
...
end program rutherford_scattering_theoretical_trajectory