...
use units
use MS1, only: random_stduniform
implicit none
...
!invidivual electron regardless of distance vie the vv_step_NF subroutine.
...
subroutine trajectory_NF(i, N, P, ou, Ne, Ns, emb, sct, Nx, Ny, Nz, atoms, Z, d0, T, d, dt, l, r, v, a)
...
real(dp) :: tk
	...
	do while ( .not.(end_embedded .or. end_scattered .or. end_iterations) )
		...
		call vv_step_NF(Ne, N, emb, Nx, Ny, Nz, atoms, Z, d, dt, r(i,:), v(i,:), a(i,:))
...
!			!Did the electron reach the bottom? 
!			else if (r(i,2) .lt. atoms(0,-Ny,0,2)) then
!				end_embedded = .true.
!				print*, "End condition for embedded electron at bottom is met"
!			!Did the electron reach the negative x side?
!			else if (r(i,1) .lt. atoms(-Nx,0,0,1)) then
!				end_embedded = .true.
!				print*, "End condition for embedded electron at -x side is met"
!			!Did the electron reach the positive x side?
!			else if (r(i,1) .gt. atoms(Nx,0,0,1)) then
!				end_embedded = .true.
!				print*, "End condition for embedded electron at +x side is met"
!			!Did the electron reach the negative z side?
!			else if (r(i,3) .lt. atoms(0,0,-Nz,3)) then
!				end_embedded = .true.
!				print*, "End condition for embedded electron at -z side is met"
!			!Did the electron reach the positive z side?
!			else if (r(i,3) .gt. atoms(0,0,Nz,3)) then
!				end_embedded = .true.
!				print*, "End condition for embedded electron at -z side is met"
			end if
			if (end_embedded) then
				Ne = Ne + 1
				emb(Ne,:) = r(i,:)
...
real(dp) :: material_height
	...
	!THE FOLLOWING COMMENT DOES NOT APPLY ANYMORE, BUT IS USEFUL
	!Acceleration due to target atoms is not computed since
	!effective distance to use mixed method (far field) is
	!greater than material dimensions used for thesis.
	!4*1.77*sqrt(2) = 10.62 A = 20.07 a0 is the maximum distance
	!from the origin to a material atom
	!30 a0 is used as effective distance
	!END OF COMMENT
	!Acceleration due to neutral atoms
	!ONLY computed if near the material zone, i.e. 
	material_height = atoms(0,0,0,2) + 0.5*d
...
!subroutine.
subroutine trajectory_MM(i,N,P,ou,Ne,Ns,emb,sct,Nx,Ny,Nz,atoms,Z,Nxb,Nyb,Nzb,max_eq,max_neq,b_neq,b_ec,b_er,d0,FFed,T,d,dt,l,r,v,a)
